...
typedef NS_ENUM(NSInteger, QTTapHandleType){
    QTTapHandleNone,
    QTTapHandlelink,
    QTTapHandleUser
};
...
@property (nonatomic, assign) QTTapHandleType  tapHandleType;//点击类型
...
- (void)prepareText{
    ...
    if(self.linkRanges.count==0){
        ...
        [self.linkRanges addObjectsFromArray:[self getRanges:@"http(s)?://([\\w-]+\\.)+[\\w-]+(/[\\w- ./?%&=]*)?"]];
    }
...
-(NSValue*)getSelectRange:(CGPoint)selectPoint{
    ...
    for (NSValue *rangeValue in self.linkRanges) {
        ...
        if (index >= range.location && index <range.location + range.length) {
            _tapHandleType = QTTapHandlelink;
            [self setNeedsDisplay];
            return rangeValue;
        }
    }
    for (NSValue *rangeValue in self.userRanges) {
        NSRange range = rangeValue.rangeValue;
        if (index >= range.location && index <range.location + range.length) {
            _tapHandleType = QTTapHandleUser;
            [self setNeedsDisplay];
            return rangeValue;
        }
    }
    _tapHandleType = QTTapHandleNone;
    return nil;
}
